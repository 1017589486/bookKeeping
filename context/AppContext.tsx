...
import { Bill, Category, Transaction, User, BillShare } from '../types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchDataForUser = useCallback(async () => {
    ...
    try {
      ...
      const [billsRes, transactionsRes, categoriesRes, billSharesRes] = await Promise.all([
        fetch(`${API_BASE_URL}/bills`, { headers }),
        fetch(`${API_BASE_URL}/transactions`, { headers }),
        fetch(`${API_BASE_URL}/categories`, { headers }),
        fetch(`${API_BASE_URL}/billShares`, { headers }),
      ]);
      const [billsData, transactionsData, categoriesData, billSharesData] = await Promise.all([
        billsRes.json(),
